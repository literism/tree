...
"""
分类器服务
使用vLLM在指定GPU上部署分类器模型，提供HTTP API
"""
import os
import argparse
from flask import Flask, request, jsonify
from classifier import SummaryClassifier, ClassificationInput
...
def classify():
    ...
    try:
        ...
        # 调用分类器
        output = classifier.classify(input_data)
        
        # 返回结果
        return jsonify({
            'selected_indices': output.selected_indices,
            'need_new_class': output.need_new_class
        })
...
def main():
    parser = argparse.ArgumentParser(description='分类器服务')
    ...
    parser.add_argument('--temperature', type=float, default=0.1, help='温度')
    parser.add_argument('--top_p', type=float, default=0.9, help='top_p')
    parser.add_argument('--max_tokens', type=int, default=128, help='最大生成token数')
